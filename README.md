...
## 직렬화(Serialization)
직렬화는 객체를 데이터 스트림으로 만드는 것을 뜻한다.
다시 말하면, 객체에 저장된 데이터를 스트림에 쓰기(write)위해 연속적인(serial) 데이터로 변환하는 것을 말한다.
반대로 스트림으로부터 데이터를 읽어서 객체를 만드는 것을 역직렬화(deserialization)라고 한다.
단순히 객체를 저장하거나 전송하기 위해 필요한 과정이다.
객체를 저장한다는 것은 객체의 인스턴스 변수의 값을 저장한다는 의미이다. (메소드는 포함되지 않음. 왜냐하면 메서드는 변하는 값이 아니기 때문에 매번 같은 코드를 메모리에 올릴 필요가 없기 때문
- ObjectInput/OutputStream : 직렬화/역직렬화에 사용한다. (Serial 예제 참고)
- Serializable : 직렬화가 가능한 클래스를 만드고자 할 때는 Serializable 인터페이스를 구현하면 된다.
